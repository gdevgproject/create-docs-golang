...
// --- TOKEN COUNTER (o200k_base — encoding thật của GPT-4o/GPT-5.x/GPT-5.6) ---
// Đặt Ở ĐÂY (trước router) vì define() là statement chạy tuần tự, không được
// PHP hoist như function — nếu đặt bên dưới router thì khi router gọi thẳng vào
// handle_generation_request(), luồng thực thi sẽ chưa bao giờ "đi qua" các dòng
// define() này trước khi cần dùng tới, gây lỗi "Undefined constant".
define('O200K_VOCAB_URL', 'https://openaipublic.blob.core.windows.net/encodings/o200k_base.tiktoken');
define('O200K_VOCAB_SHA256', '446a9538cb6c348e3516120d7c08b09f57c36495e2acfffe59a5bf8b0cfb1a2d'); // hash công bố chính thức bởi OpenAI/tiktoken
// Regex pre-tokenize CHÍNH XÁC của o200k_base (nguồn: tiktoken_ext/openai_public.py của OpenAI)
...
function handle_generation_request()
{
  ...
  // 'full'  = ghi + trả về đầy đủ nội dung (mặc định, như cũ)
  // 'stats' = vẫn ghi file ra đĩa (để tải/download được) nhưng FE sẽ không tự
  //           load nội dung khổng lồ vào textarea, đỡ nặng khi render.
  $mode = ($_GET['mode'] ?? 'full') === 'stats' ? 'stats' : 'full';
  $fh = null;
  ...
  try {
    ...
    // 🔢 Chuẩn bị bộ đếm token TRƯỚC vòng lặp — nếu vocab thật chưa được cache sẵn,
    // lần này sẽ tải về (~2.5MB, chỉ 1 lần duy nhất cho cả server), nên báo cho
    // người dùng biết đang xảy ra gì thay vì im lặng khiến họ tưởng bị treo.
    $vocabAlreadyCached = file_exists(__DIR__ . '/.o200k_base.tiktoken');
    if (!$vocabAlreadyCached) {
      send_sse('log', '🔢 Đang tải bộ vocab token chính xác (o200k_base, ~2.5MB, chỉ 1 lần duy nhất)...');
    }
    load_o200k_ranks(); // trigger tải/parse vocab 1 lần, xác định $GLOBALS['__token_mode']
    $tokenMode = $GLOBALS['__token_mode'] ?? 'estimate';
    if ($tokenMode === 'exact') {
      send_sse('log', '✅ Token counter: CHÍNH XÁC (o200k_base thật — encoding của GPT-4o/GPT-5.x/GPT-5.6).');
    } else {
      send_sse('log', '⚠️ Không tải được vocab thật (server không có internet ra ngoài hoặc bị chặn) — dùng chế độ ƯỚC TÍNH (~sai số 10-15%).');
    }
    ...
    $totalLines  = 0;
    $totalTokens = 0; // 🔢 cộng dồn token ước tính trong CÙNG vòng lặp (single-pass, không đọc lại file)
      ...
      if (isset($BINARY_EXT_MAP[$ext])) {
        ...
        if ($fileSize === false || $fileSize < 0) {
          ...
          if ($content === false) {
            ...
            // 🔢 Ước tính token NGAY ĐÂY luôn — vẫn single-pass, không đọc lại file lần nữa
            $totalTokens += estimate_tokens_gpt($content);
    ...
    $tokenLabel = $tokenMode === 'exact' ? 'tokens (o200k_base, exact)' : '~' . number_format($totalTokens) . ' tokens (estimate)';
    fwrite($fh, "\n<!-- Stats: {$totalFiles} files | " . number_format($totalLines) . " lines of code | " . ($tokenMode === 'exact' ? number_format($totalTokens) . ' ' . $tokenLabel : $tokenLabel) . " | Generated in {$elapsed}s -->");
    ...
    send_sse('complete', $fileName, [
      ...
      'lines'      => $totalLines,
      'tokens'     => $totalTokens,
      'token_mode' => $tokenMode, // 'exact' | 'estimate' — FE dùng để hiển thị đúng nhãn
      'elapsed'    => $elapsed,
      'size'       => $fileSize,
      'mode'       => $mode,
    ]);
...
/**
 * ============================================================================
 * 🔢 TOKEN COUNTER — CHÍNH XÁC (o200k_base) với fallback ƯỚC TÍNH
 * ============================================================================
 * o200k_base là encoding CHÍNH THỨC của OpenAI dùng cho cả họ model hiện đại:
 * GPT-4o, GPT-5.x (Sol/Terra/Luna, tức GPT-5.6) đều dùng chung encoding này —
 * KHÔNG phải cl100k_base cũ (dùng cho GPT-3.5/GPT-4 đời đầu).
 *
 * CÁCH LÀM ĐỂ CÓ SỐ CHÍNH XÁC 100% MÀ KHÔNG CẦN CÀI GÌ LÊN MÁY:
 *   1. Tải trực tiếp file vocab GỐC do chính OpenAI công khai (repo tiktoken):
 *      https://openaipublic.blob.core.windows.net/encodings/o200k_base.tiktoken
 *      (đây CHÍNH LÀ file mà thư viện tiktoken thật của OpenAI tải về khi chạy,
 *      không phải bản tự chế — nội dung được xác thực bằng SHA-256 trước khi dùng).
 *   2. Cache lại 1 bản duy nhất cạnh file index.php này (~2.5MB) — chỉ tải 1 LẦN
 *      DUY NHẤT, các lần generate sau đọc từ cache, không tải lại.
 *   3. Chạy đúng thuật toán Byte-Pair-Encoding merge (thuật toán gốc của
 *      tiktoken) bằng PHP thuần — không cần ext, không cần Composer/pip.
 *   4. Regex pre-tokenize dùng ĐÚNG pattern gốc mà OpenAI công bố trong
 *      tiktoken_ext/openai_public.py cho o200k_base.
 *
 * KHI NÀO KHÔNG CHÍNH XÁC: nếu server chạy file này KHÔNG có internet ra ngoài
 * (firewall chặn openaipublic.blob.core.windows.net) thì không tải được vocab
 * → tự động rớt về hàm ƯỚC TÍNH (heuristic, sai số ~10-15%) và UI sẽ hiển thị
 * rõ ràng "~" (ước tính) thay vì số chính xác, không bao giờ âm thầm sai lệch
 * mà không báo cho người dùng biết.
 * ============================================================================
 */

/**
 * Đảm bảo file vocab o200k_base.tiktoken có sẵn cục bộ (tải nếu chưa có),
 * luôn xác thực SHA-256 trước khi tin dùng (chống file hỏng/giả mạo).
 * Có cơ chế "fail cache" để không thử tải lại liên tục nếu server không có mạng
 * (tránh làm chậm mọi lần generate khi biết chắc sẽ thất bại).
 * @return string|false Đường dẫn file vocab hợp lệ, hoặc false nếu không có được.
 */
function ensure_o200k_vocab_file()
{
  $cacheFile = __DIR__ . '/.o200k_base.tiktoken';
  $failFile  = __DIR__ . '/.o200k_fail';

  // Đã có cache — xác thực lại hash mỗi lần (rẻ, ~10-20ms cho 2.5MB) để chắc chắn file không hỏng
  if (file_exists($cacheFile)) {
    $data = @file_get_contents($cacheFile);
    if ($data !== false && hash('sha256', $data) === O200K_VOCAB_SHA256) {
      return $cacheFile;
    }
    @unlink($cacheFile); // cache hỏng — xoá để tải lại
  }

  // Vừa fail gần đây (< 10 phút) — khỏi thử lại ngay, đỡ làm chậm request hiện tại
  if (file_exists($failFile) && (time() - (int)@file_get_contents($failFile)) < 600) {
    return false;
  }

  // Thử tải file vocab thật từ OpenAI
  $data = false;
  $ctx = stream_context_create(['http' => ['timeout' => 20], 'ssl' => ['verify_peer' => true, 'verify_peer_name' => true]]);
  if (ini_get('allow_url_fopen')) {
    $data = @file_get_contents(O200K_VOCAB_URL, false, $ctx);
  }
  if ($data === false && function_exists('curl_init')) {
    $ch = curl_init(O200K_VOCAB_URL);
    ...
    $result = curl_exec($ch);
    if ($result !== false && curl_getinfo($ch, CURLINFO_HTTP_CODE) === 200) $data = $result;
    curl_close($ch);
  }

  if ($data === false || strlen($data) < 1000000 || hash('sha256', $data) !== O200K_VOCAB_SHA256) {
    // Không tải được / bị chặn mạng / nội dung không khớp hash gốc → ghi fail-marker, dùng fallback ước tính
    @file_put_contents($failFile, (string)time());
    return false;
  }

  // Ghi cache theo kiểu atomic (ghi file tạm rồi rename) để tránh race condition khi nhiều request cùng lúc
...
/**
 * Parse file .tiktoken (mỗi dòng: "<base64_bytes> <rank>") thành map bytes=>rank,
 * dùng cho thuật toán BPE merge. Cache trong biến static để chỉ parse 1 lần/request
 * dù có gọi encode nhiều file trong 1 lượt generate.
 * @return array|false
 */
function load_o200k_ranks()
{
  static $ranks = null;
  static $attempted = false;
  if ($attempted) return $ranks;
  $attempted = true;

  $path = ensure_o200k_vocab_file();
  if ($path === false) {
    $GLOBALS['__token_mode'] = 'estimate';
    return $ranks = false;
  }
  ...
  while (($line = fgets($fh)) !== false) {
    ...
    $bytes = base64_decode(substr($line, 0, $sp), true);
    if ($bytes === false) continue;
    $map[$bytes] = (int)substr($line, $sp + 1);
  }
  fclose($fh);

  // An toàn: đảm bảo 256 byte đơn đều có mặt (byte-fallback token) để thuật toán merge
  // luôn hội tụ dù vocab thiếu sót gì đó bất thường.
  for ($i = 0; $i < 256; $i++) {
    if (!isset($map[chr($i)])) $map[chr($i)] = 900000 + $i;
  }

  $GLOBALS['__token_mode'] = 'exact';
  return $ranks = $map;
}

/**
 * Thuật toán BPE byte-pair-merge GỐC của tiktoken: bắt đầu từ từng byte riêng lẻ,
 * lặp lại việc gộp cặp liền kề có rank thấp nhất (ưu tiên merge cao nhất) cho tới khi
 * không còn cặp nào nằm trong vocab. Số phần tử còn lại SAU CÙNG = số token thật.
 * Có giới hạn an toàn $maxChunk để tránh sập hiệu năng với chunk cực dài bất thường
 * (vd 1 dòng minified/base64 dài hàng chục KB không hề có khoảng trắng).
 *
 * ⚡ MEMOIZE: code thật luôn có RẤT NHIỀU chunk lặp lại (từ khóa "function"/"return",
 * dấu câu "{", ";", khoảng trắng thụt lề, v.v. — theo đúng luật Zipf của ngôn ngữ/code).
 * Benchmark thực tế trên codebase ~3500 file: cache giúp nhanh hơn 1.6x (worst-case,
 * toàn hash/base64 ngẫu nhiên) tới 9x (code bình thường, nhiều từ khóa lặp lại), mà
 * kết quả vẫn giống hệt bản không cache (đây là tối ưu AN TOÀN, không đánh đổi độ chính xác).
 * Có giới hạn kích thước cache ($maxCacheEntries) để tránh phình RAM vô hạn với codebase
 * cực đoan (hàng triệu chuỗi random không trùng) — vượt ngưỡng thì tính trực tiếp,
 * không cache thêm, vẫn ra kết quả đúng, chỉ mất phần lợi ích tăng tốc từ đó trở đi.
 */
function bpe_token_count($bytes, &$ranks, $maxChunk = 512)
{
  static $cache = [];
  static $maxCacheEntries = 3000000; // ~ vài trăm MB tối đa cho cache, đủ an toàn trong 1024M memory_limit

  if (isset($cache[$bytes])) return $cache[$bytes];

  $n = strlen($bytes);
  if ($n === 0) return 0;
  if ($n === 1) {
    if (count($cache) < $maxCacheEntries) $cache[$bytes] = 1;
    return 1;
  }
  if ($n > $maxChunk) {
    $r = (int)ceil($n / 4); // fallback cục bộ chỉ cho case bất thường này
    if (count($cache) < $maxCacheEntries) $cache[$bytes] = $r;
    return $r;
  }

  $parts = str_split($bytes, 1);
  while (true) {
    $minRank = null;
    $minIdx  = -1;
    $cnt     = count($parts) - 1;
    ...
    array_splice($parts, $minIdx, 2, [$parts[$minIdx] . $parts[$minIdx + 1]]);
  }

  $result = count($parts);
  if (count($cache) < $maxCacheEntries) $cache[$bytes] = $result;
  return $result;
}

/**
 * Hàm ƯỚC TÍNH dự phòng (dùng khi KHÔNG tải được vocab thật — server không có mạng ra ngoài).
 * Sai số thường ±10-15% so với tiktoken thật, đủ dùng để ước lượng khi bất khả kháng.
 */
function estimate_tokens_heuristic($content)
{
  static $pattern = '/(?i:[sdmt]|ll|ve|re)|[^\r\n\p{L}\p{N}]?+\p{L}++|\p{N}{1,3}| ?[^\s\p{L}\p{N}]++[\r\n]*|\s*[\r\n]|\s+(?!\S)|\s+/u';
  $count = preg_match_all($pattern, $content, $matches);
  if ($count === false || $count === 0) return (int)ceil(strlen($content) / 4);

  $tokens = 0;
  foreach ($matches[0] as $m) {
    $len = strlen($m);
    if ($len === 0) continue;
    if (preg_match('/\p{L}/u', $m)) {
      $tokens += $len <= 7 ? 1 : 1 + (int)ceil(($len - 7) / 6);
    } elseif (preg_match('/^\d/', $m)) {
      $tokens += 1;
    } elseif ($m[0] === ' ' || $m[0] === "\t" || $m[0] === "\n" || $m[0] === "\r") {
      $tokens += $len <= 4 ? 1 : 1 + (int)ceil(($len - 4) / 4);
    } else {
      $tokens += $len <= 2 ? 1 : (int)ceil($len / 2);
    }
  }
  return $tokens;
}
...
/**
 * ENTRY POINT — gọi hàm này để đếm token. Tự động dùng chế độ CHÍNH XÁC (o200k_base
 * thật) nếu tải được vocab, tự động rớt về ƯỚC TÍNH nếu server không có mạng.
 * Chế độ đang dùng được lưu ở $GLOBALS['__token_mode'] ('exact' | 'estimate') để
 * hiển thị minh bạch cho người dùng ở SSE log & thanh thống kê.
 */
function estimate_tokens_gpt($content)
{
  if ($content === '' || $content === null) return 0;

  $ranks = load_o200k_ranks();
  if ($ranks !== false) {
    $c = preg_match_all(O200K_PATTERN, $content, $matches);
    if ($c !== false && $c > 0) {
      $total = 0;
      foreach ($matches[0] as $piece) {
        $total += bpe_token_count($piece, $ranks);
      }
      return $total;
    }
    // Nội dung không phải UTF-8 hợp lệ (vd binary lọt qua) — vẫn còn cách đếm chính xác
    // theo byte thô: mọi byte đơn đều có token riêng trong vocab thật (byte-fallback).
    $total = 0;
    foreach (str_split($content, 512) as $chunk) $total += bpe_token_count($chunk, $ranks);
    return $total;
  }

  // Không tải được vocab thật (server không có internet / bị firewall chặn) → ước tính
  return estimate_tokens_heuristic($content);
}
...
<body>
      ...
      <div class="preview-header">
        <div class="preview-left">
          ...
          <div class="stats-bar" id="stats-bar" style="display:none">
            ...
            <div class="stat-item" title="Chính xác nếu server tải được vocab o200k_base thật, ước tính nếu không có mạng ra ngoài">Tokens: <span id="stat-tokens">–</span></div>
            <div class="stat-item">Size: <span id="stat-size">–</span></div>
            <div class="stat-item">Time: <span id="stat-time">–</span></div>
          </div>
        </div>
        <div class="actions">
          <button class="action-btn ghost" onclick="loadFullContent()" id="btn-load" style="display:none">📄 Load Content</button>
  ...
  <script>
    ...
    let toastTimer = null;
    let lastGeneratedFile = null; // tên file .md vừa generate (dùng để load-on-demand ở chế độ Stats Only)
    let lastGenMode = 'full';
    let contentLoaded = false; // true khi textarea #editor đang chứa NỘI DUNG THẬT (không phải placeholder)
    ...
    function startGenerate() {
      ...
      const es = new EventSource(`?action=generate&path=${encodeURIComponent(dom.path.value)}&mode=${mode}`);
      ...
      es.addEventListener('complete', e => {
        ...
        // Các stat luôn hiển thị ngay, không cần chờ tải nội dung
        ...
        dom.statTokens.title = d.token_mode === 'exact'
          ? 'Chính xác — dùng đúng encoding o200k_base (GPT-4o/GPT-5.x/GPT-5.6)'
          : 'Ước tính — server không tải được vocab thật, sai số ~10-15%';
        dom.statSize.textContent = formatBytes(d.size || 0);
        dom.statTime.textContent = (d.elapsed || 0) + 's';

        // Download luôn sẵn sàng — file đã được ghi ra đĩa ở server bất kể mode nào
        dom.btnDl.href = `?download=${d.message}`;
        dom.btnDl.removeAttribute('disabled');

        if (mode === 'stats') {
          // ── STATS ONLY: không tự load nội dung khổng lồ vào textarea (đỡ nặng render) ──
          dom.editor.value = `📊 Chế độ "Stats Only" — chỉ hiển thị thống kê để tránh render nội dung nặng.\n\nBấm "📄 Load Content" ở trên (hoặc nút Copy) để tải và xem/copy toàn bộ nội dung khi cần.`;
          dom.btnLoad.style.display = '';
          dom.btnCopy.disabled = false; // Copy sẽ tự fetch nội dung khi bấm nếu chưa load
          dom.logText.textContent = '✅ Hoàn tất (chế độ Stats Only)!';
          es.close();
          dom.btnGen.disabled = false;
          setTimeout(() => dom.statusPanel.style.display = 'none', 1200);
          showToast(`✅ Done — ${d.total} files in ${d.elapsed}s (stats only)`);
        } else {
          // ── FULL: tải và hiển thị nội dung như cũ ──
    ...
    // Tải nội dung đầy đủ theo yêu cầu (dùng ở chế độ Stats Only khi người dùng
    // thật sự muốn xem/copy nội dung, thay vì tự động render ngay sau khi generate)
    async function loadFullContent() {
      if (!lastGeneratedFile) return showToast('⚠ Chưa có file nào được generate.');
      if (contentLoaded) return; // đã có sẵn rồi, khỏi tải lại

      dom.btnLoad.disabled = true;
      dom.btnLoad.textContent = '⏳ Loading...';
      try {
        const res = await fetch(`?action=get_content&file=${lastGeneratedFile}`);
        const text = await res.text();
        dom.editor.value = text;
        contentLoaded = true;
        dom.btnLoad.style.display = 'none';
        showToast('✅ Đã tải nội dung đầy đủ!');
      } catch {
        showToast('❌ Lỗi tải nội dung.');
      } finally {
    ...
    async function copyContent() {
      // Ở chế độ Stats Only, textarea có thể vẫn đang là placeholder — tự fetch
      // nội dung thật để copy mà KHÔNG cần render vào textarea (đỡ nặng UI).
      ...
      if (!contentLoaded && lastGeneratedFile) {
        try {
          ...
          return showToast('❌ Lỗi tải nội dung để copy.');
        }
      }
